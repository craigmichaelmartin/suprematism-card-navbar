...
export class CardNavbarMenuItemComponent implements OnInit {
  ...
  localState$: Observable<MenuItemStateType> = this.rawStateSource.filter(
    state => ['preSelected'].indexOf(state) > -1
  );
    ...
    const selectedBackgrounded$ = this.stateManagerService.getModel
      .filter(
        ({ activeTab, selectedTab }) =>
          activeTab &&
          activeTab !== this.supreTabId &&
          selectedTab === this.supreTabId
      )
      .mapTo('selectedBackgrounded');

    // The state stream to which template listens
    this.state$ = <Observable<MenuItemStateType>>this.localState$.merge(
      notActive$,
      active$,
      selected$,
      selectedBackgrounded$
    );
  }
